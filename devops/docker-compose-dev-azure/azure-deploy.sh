...
resourcename="erp"  # lower case, only
resourcegroup="erp_rg"
...
# git clone https://github.com/apilogicserver/erp.git
# cd erp
...
az group create --name erp_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group erp_rg --sku S1 --is-linux
az container create --resource-group erp_rg --name erp --image apilogicserver/erp:latest --dns-name-label erp --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//erp.westus.azurecontainer.io:5656'
